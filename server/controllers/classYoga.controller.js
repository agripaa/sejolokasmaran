...
const convertToEmbedUrl = require('../utils/url-youtube');
...
module.exports = {
    ...
    create: async (req, res) => {
        ...
        const embedUrl = convertToEmbedUrl(url_link);
    
        if (!embedUrl) {
          return res.status(400).json({ status: 400, msg: 'Invalid YouTube URL' });
        }
    
        try {
          const newYogaClass = await ClassYoga.create({ url_link: embedUrl });
      ...
      update: async (req, res) => {
        ...
        try {
          ...
          await ClassYoga.update({ url_link: embedUrl }, { where: { id } });
